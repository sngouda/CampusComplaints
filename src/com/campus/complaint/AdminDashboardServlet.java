    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        try (Connection conn = DBConnection.getConnection()) {
            ...
            if (priorityFilter != null && !priorityFilter.isEmpty() && !priorityFilter.equals("All")) {
                sql = "SELECT c.id, c.title, c.description, c.status, c.priority, c.created_at, s.name as student_name " +
                      "FROM complaints c JOIN students s ON c.student_id = s.id " +
                      "WHERE c.priority = ? ORDER BY c.created_at DESC";
                ps = conn.prepareStatement(sql);
                ps.setString(1, priorityFilter);
            } else {
                sql = "SELECT c.id, c.title, c.description, c.status, c.priority, c.created_at, s.name as student_name " +
